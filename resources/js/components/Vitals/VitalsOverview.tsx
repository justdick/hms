...
export default function VitalsOverview({
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-center py-12 text-gray-500">
            ...
            <p className="text-sm">Record the patient's first set of vital signs</p>
            {onAddVitals && (
  ...
  const getTrendData = (type: string) => {
    return vitals.slice(0, 5).reverse().map(vital => {
      switch (type) {
        case 'temperature':
  ...
  const bpStatus = getBPStatus(latestVitals.blood_pressure_systolic, latestVitals.blood_pressure_diastolic)
        ...
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            <CardTitle className="flex items-center gap-2">
              ...
              Vital Signs
              {hasAbnormalVitals && (
                <Badge variant="secondary" className="ml-2 text-orange-600 bg-orange-100">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <VitalCard
              ...
              lastReading={formatDateTime(latestVitals.recorded_at)}
              alert={tempStatus === 'high' ? 'Fever detected' : tempStatus === 'low' ? 'Hypothermia risk' : undefined}
            ...
            <VitalCard
              ...
              lastReading={formatDateTime(latestVitals.recorded_at)}
              alert={
                bpStatus === 'critical' ? 'Hypertensive crisis' :
                bpStatus === 'high' ? 'High blood pressure' :
                bpStatus === 'low' ? 'Low blood pressure' : undefined
              }
            ...
            <VitalCard
              ...
              lastReading={formatDateTime(latestVitals.recorded_at)}
              alert={
                hrStatus === 'high' ? 'Tachycardia' :
                hrStatus === 'low' ? 'Bradycardia' : undefined
              }
            ...
            <VitalCard
              ...
              lastReading={formatDateTime(latestVitals.recorded_at)}
              alert={
                rrStatus === 'high' ? 'Tachypnea' :
                rrStatus === 'low' ? 'Bradypnea' : undefined
              }
            ...
            <div className="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
              ...
              <div className="space-y-2 max-h-32 overflow-y-auto">
                {vitals.slice(1, 6).map((vital, index) => (
                  <div
                    key={vital.id}
                    className="flex items-center justify-between text-xs text-gray-600 dark:text-gray-400 p-2 bg-gray-50 dark:bg-gray-800 rounded"
                  >
                    <span>{formatDateTime(vital.recorded_at)}</span>
                    <div className="flex gap-4">
                      <span>T: {vital.temperature}°F</span>
