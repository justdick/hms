...
const commonTemplates: SOAPTemplate[] = [
  {
    ...
    subjective: 'Patient reports compliance with antihypertensive medications. No chest pain, shortness of breath, or dizziness. Reports regular home BP monitoring.',
    objective: 'Vital signs stable. BP improved from last visit. Cardiovascular exam unremarkable.',
  ...
  },
  {
    ...
    subjective: 'Patient monitoring blood glucose regularly. Diet adherence good. No polyuria, polydipsia, or visual changes.',
    objective: 'Weight stable. Foot exam normal. No signs of diabetic complications.',
  ...
  },
  {
    ...
    subjective: 'Patient feels well. No acute complaints. Reviews systems negative.',
...
const quickPhrases: QuickPhrase[] = [
  { abbreviation: 'wnl', expansion: 'within normal limits', category: 'general' },
  { abbreviation: 'sob', expansion: 'shortness of breath', category: 'symptoms' },
  { abbreviation: 'cp', expansion: 'chest pain', category: 'symptoms' },
  { abbreviation: 'nkda', expansion: 'no known drug allergies', category: 'general' },
  { abbreviation: 'rrr', expansion: 'regular rate and rhythm', category: 'cardiac' },
  { abbreviation: 'ctab', expansion: 'clear to auscultation bilaterally', category: 'pulmonary' },
  { abbreviation: 'nab', expansion: 'no acute distress', category: 'general' },
  { abbreviation: 'heent', expansion: 'head, eyes, ears, nose, throat', category: 'examination' },
  { abbreviation: 'rom', expansion: 'range of motion', category: 'musculoskeletal' },
...
export default function SmartSOAPNotes({
  ...
  useEffect(() => {
    ...
    const stats = Object.keys(data).reduce((acc, key) => {
      if (key.includes('notes') || key === 'chief_complaint') {
        const wordCount = data[key as keyof typeof data].split(' ').filter(word => word.length > 0).length
  ...
  const insertQuickPhrase = (phrase: QuickPhrase, fieldKey: string) => {
    const textarea = textareaRefs[fieldKey as keyof typeof textareaRefs].current
    if (!textarea) return
  ...
  const handleTextareaKeyDown = (e: React.KeyboardEvent, fieldKey: string) => {
    if (e.key === 'Tab') {
      e.preventDefault()
      ...
      const matchingPhrase = quickPhrases.find(phrase => phrase.abbreviation === lastWord)
      if (matchingPhrase) {
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Popover open={showPhrases} onOpenChange={setShowPhrases}>
              <PopoverTrigger asChild>
                <Button variant="outline" size="sm">
              ...
              <PopoverContent className="w-80">
                <div className="space-y-3">
                  <h4 className="font-medium text-sm">Quick Phrases (Press Tab to expand)</h4>
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    {quickPhrases.map((phrase) => (
                      ...
                      >
                        <code className="font-mono font-bold">{phrase.abbreviation}</code>
                        <p className="text-gray-600 truncate">{phrase.expansion}</p>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <form onSubmit={onSubmit} className="space-y-6">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex items-center gap-2 text-xs">
                <span className={getCompletionColor(completionStats.chief_complaint)}>
            ...
            <Textarea
              ...
              onChange={(e) => {
                handleDataChange('chief_complaint', e.target.value)
                autoResize(e.target)
              }}
              onKeyDown={(e) => handleTextareaKeyDown(e, 'chief_complaint')}
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex items-center gap-2 text-xs">
                <span className={getCompletionColor(completionStats.subjective_notes)}>
            ...
            <Textarea
              ...
              onChange={(e) => {
                handleDataChange('subjective_notes', e.target.value)
                autoResize(e.target)
              }}
              onKeyDown={(e) => handleTextareaKeyDown(e, 'subjective_notes')}
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex items-center gap-2 text-xs">
                <span className={getCompletionColor(completionStats.objective_notes)}>
            ...
            <Textarea
              ...
              onChange={(e) => {
                handleDataChange('objective_notes', e.target.value)
                autoResize(e.target)
              }}
              onKeyDown={(e) => handleTextareaKeyDown(e, 'objective_notes')}
              className="min-h-[100px] resize-none"
            />
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex items-center gap-2 text-xs">
                <span className={getCompletionColor(completionStats.assessment_notes)}>
            ...
            <Textarea
              ...
              onChange={(e) => {
                handleDataChange('assessment_notes', e.target.value)
                autoResize(e.target)
              }}
              onKeyDown={(e) => handleTextareaKeyDown(e, 'assessment_notes')}
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex items-center gap-2 text-xs">
                <span className={getCompletionColor(completionStats.plan_notes)}>
            ...
            <Textarea
              ...
              onKeyDown={(e) => handleTextareaKeyDown(e, 'plan_notes')}
              className="min-h-[100px] resize-none"
            />
          </div>
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) => handleDataChange('follow_up_date', e.target.value)}
                  ...
                  >
                    ...
                    {note.assessment_notes && (
                      <div className="text-gray-600 dark:text-gray-400 mt-1">
                        A: {note.assessment_notes.substring(0, 100)}...
                      </div>
                    )}
                  </div>
                ))}
