...
interface CoverageRule {
    ...
    max_quantity_per_visit?: number;
    max_amount_per_visit?: string;
...
export default function CoverageRuleEdit({ rule, plans }: Props) {
    const { data, setData, put, processing, errors } = useForm({
        insurance_plan_id: rule.insurance_plan_id?.toString() || '',
        coverage_category: rule.coverage_category || '',
        item_code: rule.item_code || '',
        item_description: rule.item_description || '',
        is_covered: rule.is_covered ?? false,
        coverage_type: rule.coverage_type || '',
        coverage_value: rule.coverage_value || '',
        patient_copay_percentage: rule.patient_copay_percentage || '',
        max_quantity_per_visit: rule.max_quantity_per_visit?.toString() || '',
        max_amount_per_visit: rule.max_amount_per_visit || '',
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(`/admin/insurance/coverage-rules/${rule.id}`);
    };
