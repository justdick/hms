...
const statusConfig: Record<string, { label: string; color: string; icon: React.ReactNode }> = {
    ...
    paid: { label: 'Paid', color: 'bg-emerald-600', icon: <DollarSign className="h-4 w-4" /> },
    partial: { label: 'Partial', color: 'bg-orange-500', icon: <RefreshCw className="h-4 w-4" /> },
};
...
export default function ClaimsSummary({ data, providers, filters }: Props) {
    ...
    const handleApplyFilters = (e: FormEvent) => {
        e.preventDefault();
        router.get('/admin/insurance/reports/claims-summary', localFilters as any, {
            preserveState: true,
            preserveScroll: true,
        });
    };
    ...
    const handleClearFilters = () => {
        ...
        router.get('/admin/insurance/reports/claims-summary', defaultFilters as any, {
            preserveState: true,
            preserveScroll: true,
        });
    };
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="mt-1 text-gray-600 dark:text-gray-400">
                                Overview of all claims with status breakdown for the selected period
                            </p>
                        </div>
                    </div>
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className="grid grid-cols-2 gap-4 md:grid-cols-4 lg:grid-cols-8">
                            {Object.entries(data.status_breakdown).map(([status, breakdown]) => (
                                <div
                                    key={status}
                                ...
                                >
                                    ...
                                    <p className="text-2xl font-bold">{breakdown.count}</p>
                        ...
                        {data.claims_by_provider.length > 0 ? (
                            <div className="overflow-x-auto">
                                <Table>
                                    ...
                                    <TableBody>
                                        {data.claims_by_provider.map((provider) => (
                                            <TableRow key={provider.id}>
                                                ...
                                                <TableCell className="text-right">
                                                    <Badge variant="outline">{provider.claim_count}</Badge>
                                                </TableCell>
                                                <TableCell className="text-right">
                                                    {formatCurrency(provider.total_claimed)}
                                                </TableCell>
                                                <TableCell className="text-right text-green-600">
                                                    {formatCurrency(provider.total_approved)}
                                                </TableCell>
                                                ...
                                                <TableCell className="text-right text-red-600">
                                                    {formatCurrency(
                                                        (provider.total_approved || 0) - (provider.total_paid || 0)
                                                    )}
                                                </TableCell>
                                            </TableRow>
                                        ))}
